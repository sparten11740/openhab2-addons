...
public class EdimaxBindingConfiguration implements BindingConfig {
    ...
    public void parse(Item item, String bindingConfig) throws BindingConfigParseException {
        // TODO: this needs to be reworked, for now it's sufficient to use the discovery service
        ...
        if (configParts.length < 1) {
            throw new BindingConfigParseException(
                    "Edimax configuration must contain ip address. Optionally the password is to be specified, separated with ':'. And optionally it contains the type (separated from password by a ':'). Available types are: 'POWER', 'CURRENT' and 'STATE' which is default.");
        }

        ipAddress = configParts[0];
        ...
        if (!item.getClass().equals(validClass)) {
            throw new BindingConfigParseException(
                    "Type: '" + type + "' only supports " + validClass.getSimpleName() + ".");
        }
    }
